window.onload = function() 
{
    ...
    let nom;
    let count = 5; //nombre de vie au debut du jeu
    const countConstante = count;
    const tableau = [];
    const difficult = [10,20,30,50];
    let niveau=0;
    let succes=false;
    ...
    const idBouton1 = document.getElementById("bouton1"); 
    ...
    function intro() //Valide le pseudo et creation de la trame de jeu
    {
     //------------------------Fonction Bienvenue----------------------------------------------
        ...
        audio.play();
        nom = idInput1.value;
        const welcome="<p><strong>Mode Facile: </strong>" 
                    +"<b>" 
                    +nom 
                    +"</b>" 
                    +" tu as <b>"
        ...
        else
        {
            ...
            else
            {
                ...
                if (tableau[countConstante - count] == hasard && count > 0) 
                {
                    ...
                    const bravo2="Bravo " 
                              +"<strong>" +nom +"</strong>" 
                              +"!\nTu as reussi tous les niveaux du mode <strong>Facile!!!</strong>"
                              +"\nEn "
    ...
    //-------------------Gestion du bouton changer de niveau/rejouer-----------------------------    
                    ...
                    else
                    {
                        ...
                        idBouton3.textContent="Niveau Moyen";
                        idBouton3.style.visibility = "visible";
                    }
    ...
    //-----------------------------Mise à jour Vie-----------------------------------------------    
        ...
        vie[countConstante - count].src = 'img/coeurbarre.png';
        const erreurNombre = document.querySelectorAll("#ligneNombre .nombreJoueur");
        erreurNombre[countConstante - count].style.background = "url('img/casebarre.png') center /100% 100%";
...
//-----------------------------------------------------------------------------------------------
    function rejouer() 
    {
        if(niveau==3 && succes==true)
            window.location="moyen.html";
...
//-----------------------------Lancement du jeu via addlistener---------------------------------
    idBouton1.addEventListener('click', intro);
    idInput1.onkeyup = function() {if (event.keyCode == 13)intro();}; //valide par touche entrée
